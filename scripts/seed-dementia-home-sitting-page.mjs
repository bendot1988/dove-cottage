...
import data from "../src/data/dementia-home-sitting.json" with { type: "json" };
...
await client.createOrReplace({
  _id: "dementiaHomeSittingPage.main",
  _type: "dementiaHomeSittingPage",
  ...
  sectionImage1: { image: sectionImg, alt: data.sectionImage1?.alt },
  serviceHeading: data.serviceHeading,
  serviceIntro: data.serviceIntro,
  serviceOfferHeading: data.serviceOfferHeading,
  serviceOfferList: withKeys(data.serviceOfferList, "offer"),
  serviceOutro: data.serviceOutro,
  whoHeading: data.whoHeading,
  whoList: withKeys(data.whoList, "who"),
  helpHeading: data.helpHeading,
  helpCards: withKeys(
    (data.helpCards ?? []).map((card) => ({
      ...card,
      bullets: withKeys(card.bullets, `help-${card.title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-bullet`),
    })),
    "help-card"
  ),
  importantHeading: data.importantHeading,
  importantList: withKeys(data.importantList, "important"),
  importantOutro: data.importantOutro,
  flexHeading: data.flexHeading,
  flexIntro: data.flexIntro,
  flexList: withKeys(data.flexList, "flex"),
  referralsHeading: data.referralsHeading,
  referralsBody1: data.referralsBody1,
  referralsBody2: data.referralsBody2,
...
console.log("Seeded Sanity document: dementiaHomeSittingPage.main");
